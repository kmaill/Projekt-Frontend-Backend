    ...
    <div className="min-h-[80vh] flex items-center justify-center p-4 dark:text-gray-100 transition-colors duration-200">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl w-full max-w-md border border-gray-100 dark:border-gray-700 transition-colors duration-200">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{t('loginPage.welcomeBack')}</h2>
          <p className="text-gray-500 dark:text-gray-400">{t('loginPage.loginToManage')}</p>
        </div>

        <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('loginPage.emailLabel')}</label>
        ...
        <div className="mt-6 flex items-center">
          ...
          <span className="px-3 text-sm text-gray-500 dark:text-gray-400">{t('loginPage.orLoginWith')}</span>
          <div className="flex-grow border-t border-gray-300 dark:border-gray-600"></div>
        </div>
        ...
        <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-8">
          {t('loginPage.noAccount')} <Link to="/register" className="text-emerald-600 dark:text-emerald-400 font-semibold hover:underline">{t('loginPage.registerLink')}</Link>
        </p>
      </div>
    </div>
  );
